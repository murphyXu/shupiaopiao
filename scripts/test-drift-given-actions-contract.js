...
assert.ok(givenWxml.includes('class="order-action primary"') && givenWxml.includes('class="order-action secondary"') && givenWxml.includes('class="order-action danger"'), 'given list action buttons should share one base action style');
assert.ok(givenWxml.includes('查看寄送信息') && !givenWxml.includes('>查看详情<'), 'given list should rename detail entry to shipping info');
assert.ok(givenWxml.includes('bindtap="copyShippingInfo"') && givenWxml.includes('bindtap="cancel"') && givenWxml.includes('取消漂流'), 'given list should copy shipping info and expose cancellation');
assert.ok(givenWxss.includes('.order-action') && givenWxss.includes('.order-action.primary') && givenWxss.includes('.order-action.secondary') && givenWxss.includes('.order-action.danger'), 'given list should define consistent action button styles');
assert.ok(givenJs.includes('copyShippingInfo(e)') && givenJs.includes("api.getOrderDetail(orderId, 'given')") && givenJs.includes('wx.setClipboardData'), 'given shipping info action should copy address from detail data');
assert.ok(givenJs.includes('hasShippingInfo(address)') && givenJs.includes('shippingInfoText(address)'), 'given shipping info action should reject empty address snapshots only after field validation');
...
assert.ok(detailJs.includes('copyShippingInfo()') && detailJs.includes('wx.setClipboardData'), 'order detail page should copy receiver shipping information');
assert.ok(detailJs.includes('hasShippingInfo(address)') && detailJs.includes('shippingInfoText(address)'), 'order detail page should validate address snapshot fields before copying');
